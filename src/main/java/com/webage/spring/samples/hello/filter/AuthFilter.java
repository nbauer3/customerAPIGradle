package com.webage.spring.samples.hello.filter;
...
import com.webage.spring.samples.hello.util.JWTHelper;
...
public class AuthFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// CAN REQUEST forAll() with authentication but not by name or id specifically with authentication
		if(
	       !uri.equals("/Customer")
	       && !uri.equals("/Customer/name")
	       && !uri.equals("/")
	       ) {
			chain.doFilter(request, response);
			return;			
	 ...
	 * }
	 * 
	 * @Override public void destroy() { ApiLogger.log("AuthFilter.destroy"); }
	 */
